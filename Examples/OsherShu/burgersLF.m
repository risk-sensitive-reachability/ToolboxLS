...
%   dimensional version of Burgers' equation (an HJ PDE with an H function
%   convex in \grad \phi).
%
...
%       \phi(x, 0) = -\cos((\pi / dimension) * s\um_i x_i)
data = g.xs{1};
for i = 2 : g.dim
  data = data + g.xs{i};
end
...
schemeData.grid = g;
schemeData.hamFunc = @burgersHamFunc;
schemeData.partialFunc = @burgersPartialFunc;
...
%
% This function implements the hamFunc prototype to calculate the 
%   analytic Hamiltonian for Burgers' equation, from Examples 1 & 2
%   of Osher & Shu 1991.
%
% Specifically, H(x,p) = 0.5 * (\alpha + \sum_i p_i)^2
%
% Parameters:
...
%   Hamiltonian with respect to the costate (gradient) for Burgers'
%   equation, from Examples 1 & 2 of Osher & Shu 1991.
%
...
%   where H(x,p) = 0.5 * (\alpha + sum_i p_i)^2.
%
% So we are maximizing over p the function |\alpha + \sum_i p_i|.
%
% Parameters:
%   t            Time at beginning of timestep (ignored).
%   data         Data array.
...
% For simplicity we'll sum the absolute value of each component
%   (it may overestimate the extrema of the partial).
sumAbs = zeros(size(data));
for i = 1 : schemeData.grid.dim
  sumAbs = sumAbs + max(abs(derivMin{i}), abs(derivMax{i}));
end

alpha = schemeData.alpha + sumAbs;
